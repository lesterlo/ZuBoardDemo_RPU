...
int32_t platform_init(int32_t argc, char *argv[], void **platform)
{
	...
	if (!platform) {
		xil_printf("Failed to initialize platform, NULL platform pointer.\r\n");
		return -EINVAL;
	}
...
int32_t platform_poll_on_vdev_reset(void *arg)
{
	...
	while (!virtio_get_status(rvdev->vdev, &vdev_status) &&
	       (vdev_status & VIRTIO_CONFIG_STATUS_DRIVER_OK)) {
#ifdef RPMSG_NO_IPI
		(void)flags;
